...
class UserRoleSprunjeTest extends AdminTestCase
{
    use RefreshDatabase;
    use WithTestUser;
    ...
    public function testPageForGuestUser(): void
    {
        ...
        $request = $this->createJsonRequest('GET', '/api/users/u/foo/roles');
    ...
    public function testPage(): void
    {
        /** @var User */
        $user = User::factory()->create();
        $this->actAsUser($user, permissions: ['view_user_roles']);

        // Create request with method and url and fetch response
        $request = $this->createRequest('GET', '/api/users/u/' . $user->user_name . '/roles');
        $response = $this->handleRequest($request);
